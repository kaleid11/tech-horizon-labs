...
- **410 Gone for dead URLs**: `gone()` helper in `server/routes.ts` returns 410 for permanently removed WordPress-era paths (`/category/*`, `/tag/*`, `/page/*`, `/wp-content/*`, `/wp-includes/*`, `/wp-admin*`, `/wp-login.php`, `/xmlrpc.php`, `/feed*`, `/comments/feed`, `/trackback`, and unknown `/blog/:slug`). Lets Google deindex faster than soft-404 catch-all redirects.
...
Deep analysis of 10 AI companies — Anthropic, OpenAI, Google DeepMind, Meta AI, xAI, DeepSeek, Qwen (Alibaba), Perplexity, Kimi, and Mistral. Features a three-view comparison dashboard (Valuation Race multi-line SVG chart with gradient area fills, Total Funding horizontal bar chart, Quick Stats grid with Revenue/Compute Spend), cumulative funding raised line chart, expandable company profile cards with tabbed UI (Overview/Funding or History/Investors), and at-a-glance comparison table with Model Access, Revenue, and Compute Spend columns. All data embedded in `client/static/research.html`. SVG charts use `buildLineChart()` in `main.js` with gradient `<defs>`, interactive tooltips, and click-to-highlight legends. Anthropic valuation data verified: Series B $4B (Apr 2022), Google Strategic $4.6B (Feb 2023), Series C $4.1B (May 2023). DeepSeek profile includes V3 671B MoE architecture, R1 $5.6M training cost, sanctions context. Qwen profile includes Zhou Jingren leadership, Qwen2.5 model lineup (7 sizes), QwQ reasoning model. Kimi (Moonshot AI) model access is "Open Weights" (K2+ models on Hugging Face). FAQPage JSON-LD schema with open source vs open weights distinction. Animations (dot-grid canvas and fade-in scroll effects) are disabled on the research page for readability. Company colors: Anthropic #d4a843, OpenAI #10A37F, Google DeepMind #4285F4, Meta AI #0668E1, xAI #1DA1F2, DeepSeek #E53935, Qwen #FF6A00, Perplexity #20B8CD, Kimi #6366F1, Mistral #FF7000.
...
#### Analytics
- **Plausible Analytics**: Installed across all 34 HTML pages via `pa-dRBvledtC13XyS9b9WRTT.js` script in `<head>`. Pageview tracking only (no custom events configured yet).
...
- Sitemap with 34 URLs
- FAQPage JSON-LD on key pages (homepage, assessment, scorecard, tools, location pages, and 7 of 10 articles — the 3 oldest articles lack FAQ content sections)
