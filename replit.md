...
- **AI Readiness Report (`/report`)**: Gated landing page for "State of AI Readiness: Australian SMB 2026" PDF. Stats: $44B GDP opportunity, 5% fully enabled, 4.1x AU over-index, 61% ChatGPT Plateau, 111% profitability uplift. Collects email via `/api/newsletter` with `source: "report-download"`, stores to newsletter_signups, sends PDF download email + assessment CTA via `sendNewsletterWelcome`, syncs to Klipy and Beehiiv with `report-download` tag. Already-subscribed users get the email re-sent. PDF at `client/static/ai-readiness-report-2026.pdf`. CTAs on homepage and about page.
- **AI Readiness Assessment (`/assessment`)**: 10-question multi-step assessment scoring 0–100 across AI maturity dimensions. Results map to 4 stages (Discovery, ChatGPT Plateau, Systematically Enabled, Fully AI-Native) with personalised recommendations. Contact gate before results; calls `/api/audit` which stores to `audit_submissions` table, emails results via `sendAuditResults` (rich HTML email with stage name, recommendations, booking CTA, and report download CTA), notifies Huxley via `sendAuditNotification`, syncs to Klipy and Beehiiv with `ai-readiness-assessment` tag. Old `/audit-tool` URL redirects here. CTAs on homepage and contact page. Stage names align with THL's 2026 SMB research framework.
